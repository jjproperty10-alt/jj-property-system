...
describe('Vm1OperationsView', () => {
  ...
  it('renders Draft admission review without totals or partner split', () => {
    ...
    expect(html).toContain('No effective stored Owner Statement is attached for this period')
    expect(html).not.toContain('Owner Statement evidence not stored')
    expect(html).toContain('data-testid="vm1-os-evidence-missing"')
  ...
  it('shows staff-only Evidence source verified and Draft candidate from injected test Owner Statement lines', () => {
    ...
    expect(html).not.toContain('25%')
    expect(html).not.toContain('594.25')
    expect(html).not.toContain('לא נמצא Owner Statement אפקטיבי לתקופה זו')
