...
function successPayload(netOwnerPayout: number | string) {
  return {
    ok: true,
    listing_id: '412148',
    ...
    lines: [
      {
        ...
        currency: 'EUR',
        net_owner_payout: netOwnerPayout,
      },
    ],
  }
}
...
describe('readVm1PartnershipOwnerStatementForListing', () => {
  ...
  it('rejects a 1.999 number that is not exact to two decimal places', async () => {
    const { client } = jwtClient(successPayload(1.999))
    const read = await readVm1PartnershipOwnerStatementForListing({
      client,
      listingId: VM1_HOSTAWAY_LISTING_ID,
      canonicalPropertyId: VM1_CANONICAL_PROPERTY_ID,
    ...
    expect(read.kind).toBe('unavailable')
    expect(read.reason).toBe(VM1_OS_EVIDENCE_REASON.malformed)
    expect(JSON.stringify(read)).not.toContain('1.999')
    expect(JSON.stringify(read)).not.toContain('2.00')
  })

  it('rejects a "1.999" string that is not exact to two decimal places', async () => {
    const { client } = jwtClient(successPayload('1.999'))
    const read = await readVm1PartnershipOwnerStatementForListing({
      client,
      listingId: VM1_HOSTAWAY_LISTING_ID,
  ...
  it('accepts exact two-decimal amounts 2, 2.0, and "2.00"', async () => {
    for (const netOwnerPayout of [2, 2.0, '2.00']) {
      const { client } = jwtClient(successPayload(netOwnerPayout))
      const read = await readVm1PartnershipOwnerStatementForListing({
        client,
        listingId: VM1_HOSTAWAY_LISTING_ID,
        canonicalPropertyId: VM1_CANONICAL_PROPERTY_ID,
      ...
      if (!read.ok) return
      expect(read.lines[0]?.netOwnerPayoutEur).toBe(2)
    }
  })
  ...
  it('does not import ingest or void wrappers', () => {
    ...
    expect(src).toContain('if (amount !== rounded) return null')
  })
})
