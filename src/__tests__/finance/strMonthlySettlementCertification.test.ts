...
describe('Uriel Neer dry run', () => {
  const dry = JSON.parse(
    readFileSync(
      join(process.cwd(), 'artifacts/str-monthly-settlement/uriel-neer-2026-06-01-2026-08-31.dry-run.json'),
      'utf8',
    ),
  ) as {
    submitted: boolean
    duplex: { status: string }
    not_allocated: { amount_eur: string }
    payload: {
      p_total_owner_net: number
      p_lines: Array<{
        owner_net: number
        reservation_count: number
        nights: number
        component_reconciliation_status: string
  ...
  test('monthly cents sum to the certified total and duplex stays blocked', () => {
    const cents = dry.payload.p_lines.reduce((sum, line) => sum + Math.round(line.owner_net * 100), 0)
    expect(cents).toBe(Math.round(dry.payload.p_total_owner_net * 100))
    expect(cents).toBe(343498)
    expect(dry.submitted).toBe(false)
    expect(dry.duplex.status).toBe('blocked')
    expect(dry.not_allocated.amount_eur).toBe('259.02')
    expect(dry.payload.p_lines).toHaveLength(3)
    expect(dry.payload.p_lines.every((line) => line.component_reconciliation_status === 'partial')).toBe(true)
    expect(dry.payload.p_lines.every((line) => line.source_authority === 'approved_reconstruction')).toBe(true)
    expect(dry.payload.p_reason).toMatch(/grouped by check-in month/)
    expect(dry.payload.p_reason).toMatch(/not reconstructed from incomplete PMS components/)
    expect(dry.payload.p_reason).toMatch(/No guest information is included/)
    expect(JSON.stringify(dry)).not.toMatch(/guest_name|guest_email|guest_phone/)
    expect(JSON.stringify(dry.payload.p_lines)).not.toMatch(/6983\.10|259\.02|48901\.54/)
  })
})
