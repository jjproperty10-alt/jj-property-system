 ...
 * No double counting. Final identity remains Avi is owed €740.94.
 */
import {
...
describe('option A — internet split and certified department totals', () => {
  ...
  it('Setup is €5,706.06 / Avi €2,853.03 and Operations is €9,386.80 / Avi €4,693.40', () => {
    ...
    expect(roundEur(AVI_AIRBNB_SETUP_TOTAL_EUR + AVI_AIRBNB_OPERATIONS_TOTAL_EUR)).toBe(15092.86)
  })

  it('bills internet once per month for 16 months, licence 15, pool 20', () => {
    ...
    expect(internet).toHaveLength(16)
    expect(new Set(internet).size).toBe(16)
    expect(internet[0]).toBe('2025-06')
    expect(internet[internet.length - 1]).toBe('2026-09')
    expect(licence).toHaveLength(15)
    expect(new Set(licence).size).toBe(15)
    expect(pool).toHaveLength(20)
    expect(new Set(pool).size).toBe(20)
    expect(roundEur(internet.length * 30)).toBe(480)
    expect(roundEur(licence.length * 40)).toBe(600)
    expect(roundEur(pool.length * 120)).toBe(2400)
  })

  it('charges €15 only for the 29 completed stays and replaces €729.67 of supplies', () => {
    ...
    expect(roundEur(AVI_HOSTAWAY_STAY_COUNT * AVI_PER_STAY_CONSUMABLES_EUR)).toBe(435)
    ...
    expect(roundEur(consumables.reduce((s, l) => s + l.amountEur, 0))).toBe(435)
  })
})

describe('certified identity remains Avi is owed €740.94', () => {
  ...
  it('Paid + Credits − Obligation = +€740.94 and the legacy −€380.50 fails closed', () => {
    ...
    expect(summary.obligationTotalEur).toBe(299603.5)
    expect(summary.paidTotalEur).toBe(280600)
    expect(summary.creditsTotalEur).toBe(19744.44)
    expect(summary.netEur).toBe(740.94)
    expect(summary.aviIsOwed).toBe(true)
    expect(summary.rows.find((r) => r.key === 'airbnb_setup')?.obligationEur).toBe(2853.03)
    expect(summary.rows.find((r) => r.key === 'airbnb_operations')?.obligationEur).toBe(4693.4)
  })
  ...
  it('monthly view uses the same Setup and Operations totals', () => {
    ...
    expect(monthly.totals.operationsEur).toBe(9386.8)
    expect(monthly.totals.consumablesEur).toBe(435)
    expect(monthly.totals.internetEur).toBe(480)
    expect(monthly.totals.hostawayLicenceEur).toBe(600)
    expect(monthly.totals.poolServiceEur).toBe(2400)
    expect(monthly.totals.stayCount).toBe(29)
