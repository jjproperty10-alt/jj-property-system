...
describe('Avi print view — canonical 202-row certified compose', () => {
  ...
  it('keeps exact certified identity', () => {
    ...
    expect(report.partners.find((p) => p.partner === 'Avi')!.creditsEur).toBe(19495.25)
    expect(report.partners.find((p) => p.partner === 'Avi')!.obligationEur).toBe(299501)
    expect(report.partners.find((p) => p.partner === 'Avi')!.netEur).toBe(594.25)
    expect(report.partners.find((p) => p.partner === 'Avi')!.semanticNet).toBe('Avi is owed €594.25')
    expect(html).toContain('Avi is owed €594.25')
    expect(html).toContain('280,600.00')
    expect(html).toContain('19,495.25')
    expect(html).toContain('299,501.00')
    expect(html).toContain('Avi owes €11,107.07')
    expect(html).toContain('Avi is owed €12,051.32')
  ...
  it('expenses remain exactly 179 rows and reconcile to €99,002.00 / €49,501.00', () => {
    ...
    expect(html).toContain('49,501.00')
    const recon = html.split('data-testid="avi-print-expense-recon"')[1] ?? ''
    expect(recon).toContain('99,002.00')
    expect(recon).toContain('49,501.00')
  })
