...
## CLAUDE Dependency Gate — status: RELEASED
No section may be removed or trimmed from legacy `CLAUDE.md` until **every canonical reference to that section** has been migrated to a durable Git-backed authority/evidence document. **P-LEDGER / §13.17 — RESOLVED (Phase 2A):** promoted to Git as `docs/governance/JJ_LEDGER_CONSTITUTION.md` (P-LEDGER-1…6 + D4) and `docs/governance/JJ_INVESTIGATION_CONSTITUTION.md` (P-EVIDENCE-1 + Resolution Governance); P-LEDGER-7 draft excluded. **§4 + §14 — RESOLVED (Phase 2B):** current business/accounting authority promoted to Git (`docs/governance/JJ_ACCOUNTING_RULES.md` + `docs/governance/JJ_BUSINESS_RULE_BOOK.md`); `JJ_FINANCE_RULES.md` repointed (Management-Fee payer guard + Category+Subcategory preserved); durable §14 agent/process items → `JJ_AGENT_RULES.md`; stale/status §14 items excluded (not promoted). **§5 (PBI note) + §13.x (changelog) confirmed SOFT provenance** — no promotion needed. **Dependency audit COMPLETE (Phase 2B.1):** ZERO remaining current-authority dependencies on `CLAUDE.md` in `docs/canonical/` or `docs/governance/` — every §13.17/§4/§14 reference is provenance/origin; §5 (PBI note) and §13.x (changelog) are soft. The last stale pointer (`JJ_PEOPLE_AND_ENTITIES` → §4) was repointed to Git. **CLAUDE Dependency Gate = RELEASED (Yossi, 2026-08-11).** Release basis: all live business/accounting/ledger authority is Git-backed; remaining `CLAUDE.md` references are provenance/history only (zero current-authority dependencies, re-verified on main). **Release does NOT authorize deletion or archival of `CLAUDE.md`** — the **Canonical Knowledge Gate remains ACTIVE** and continues to guard archival. A lean `CLAUDE.md` refactor (against the preserved original) is now permitted as a separate future step — **not started here.**
