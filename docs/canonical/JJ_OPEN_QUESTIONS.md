...
No section may be removed or trimmed from legacy `CLAUDE.md` until **every canonical reference to that section** has been migrated to a durable Git-backed authority/evidence document. **Known dependency:** `JJ_FINANCE_RULES.md` points to `CLAUDE.md §13.17` (P-LEDGER 1–6). P-LEDGER full text must be promoted into Git (e.g. `docs/canonical/` or `docs/governance/`) **before** CLAUDE.md is trimmed — otherwise the trim breaks a canonical pointer. **CLAUDE.md must not be replaced while this gate holds.**
