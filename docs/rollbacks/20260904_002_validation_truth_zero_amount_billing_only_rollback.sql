-- =====================================================================
-- ROLLBACK — Validation Truth zero_amount billing-only exclusion
-- Manual only. Do NOT place under supabase/migrations/.
-- Restores the exact prior Production view predicate for zero_amount:
--   WHERE transactions.amount_eur <= 0
-- =====================================================================
...
UNION ALL
 SELECT transactions.id,
    ...
    'high'::text AS severity
   FROM public.transactions
  WHERE (transactions.amount_eur <= (0)::numeric)
UNION ALL
 SELECT t1.id,
    t1.date,
    t1.amount_eur,
