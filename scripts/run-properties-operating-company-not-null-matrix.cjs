...
function buildSql() {
  const migration = body('supabase/migrations/20260926120000_properties_operating_company_not_null.sql')
  const rollback = body('supabase/rollbacks/20260926120000_properties_operating_company_not_null_rollback.sql')
  ...
  const matrix = body('supabase/tests/20260926120000_properties_operating_company_not_null_matrix.sql')
    .replaceAll('@@MIGRATION@@', migration)
    .replaceAll('@@ROLLBACK@@', rollback)
  return `
BEGIN;
CREATE TEMP TABLE phase23_fp AS
SELECT
...
SELECT json_build_object(
  'failed', (SELECT count(*) FROM phase23_matrix WHERE NOT ok),
  'company_rows', (SELECT count(*) FROM registry.companies),
  'jj_properties', (SELECT count(*) FROM public.properties WHERE operating_company_id = '10f6e9b3-c5b9-4d95-a318-48f20f89477f'),
  'jj_definitions', (SELECT count(*) FROM public.property_definitions WHERE operating_company_id = '10f6e9b3-c5b9-4d95-a318-48f20f89477f'),
  'nullable_columns', (
    SELECT count(*)
...
if (require.main === module) {
  ...
  const out = path.join(process.env.TEMP || '/tmp', 'jj-p23-matrix-run.sql')
