-- Fixture data for the September pack (pack 2): the cutoff-pack pre-image, plus the five
-- September rows exactly as Production holds them (real ids, Hebrew descriptions, the
-- client_charge on the commission row), plus JJ management-fee income of EUR 25,904.05.
...
INSERT INTO public.transactions (id, date, property_id, property_name, category, subcategory, payer, payee, amount_eur, client_charge, description, notes)
VALUES
  ('79a153ee-54f7-5893-ad27-682d4bc62584', '2026-08-26', 'b54e015f-05f9-4f75-a3e7-9362c6e2e0ae', 'Tamir Kiti 1', 'Management', 'Deposit', 'Tenant', 'JJ', 550, NULL,
   $sept$התקבלו 550 אירו לחשבון jj רבולוט כדפוזית לקיטי 1 בדרום תמיר$sept$, 'WA import'),
  ('896c3124-4061-5cb8-b319-32744da183c6', '2026-09-01', 'b54e015f-05f9-4f75-a3e7-9362c6e2e0ae', 'Tamir Kiti 1', 'Management', 'Tenant Payment', 'Tenant', 'JJ', 700, NULL,
   'Kiti 1 September rent', 'WA import'),
  ('c11cd8c4-bd12-5501-8f64-66ec1de093d0', '2026-09-01', 'b54e015f-05f9-4f75-a3e7-9362c6e2e0ae', 'Tamir Kiti 1', 'Management', 'Deposit', 'Tenant', 'JJ', 50, NULL,
   $sept$קיטי 1 בדרום — מקדמת חשמל 50 מול מונה משני (לא שכירות ולא הכנסת חשמל סופית)$sept$, 'WA import'),
  ('1aecae2c-13e0-5fcc-a64d-8a5b003bed62', '2026-09-03', 'b54e015f-05f9-4f75-a3e7-9362c6e2e0ae', 'Tamir Kiti 1', 'JJ', 'Brokerage', 'Jacob', 'company', 500, 500,
   $sept$עמלת תיווך תמיר קיטי 1 בדרום 500 — charge owner Tamir once. Payee unnamed (ledger company).$sept$, 'WA import'),
  ('4fa5ba3a-5f49-5a1a-8db4-f4fbaba2ef31', '2026-09-14', '4dc5c26c-ce0c-49ec-8f44-a525f68f0b8a', 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Yossi', 800, NULL,
   'Kiti 2 September rent', 'WA import');

-- Kiti 2: 14 tenant payments through cutoff = EUR 11,716.86, property_id NULL (historical).
-- Deliberately contains NO July-2026 row and NO EUR 85 row: pack 1 creates both.
INSERT INTO public.transactions (date, property_id, property_name, category, subcategory, payer, payee, amount_eur)
VALUES
  ('2025-01-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Yossi',      800),
  ('2025-02-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia',  800),
  ('2025-03-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia',  800),
  ('2025-04-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Yossi',      800),
  ('2025-05-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia',  800),
  ('2025-06-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia',  800),
  ('2025-07-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Yossi',      800),
  ('2025-08-05', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia',  800),
  ('2026-01-10', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia', 516.86),
  ('2026-02-10', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Yossi',      800),
  ('2026-03-10', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia',  800),
  ('2026-04-10', NULL, 'Tamir Kiti 2', 'Management', 'Tenant Payment', 'Tenant', 'Anastasia',  800),
...
SELECT
  ...
  'company',
  1
FROM generate_series(1, 2236) g;

-- JJ management-fee income on OTHER properties: 34 rows summing to EUR 25,904.05, the live
-- Production pre-image (2026-09-16 22:41 UTC) that the September pack guards at Apply time.
-- Non-zero on purpose: a pack that silently zeroed this layer would pass a 0 = 0 comparison.
INSERT INTO public.transactions (date, property_id, property_name, category, subcategory, payer, payee, amount_eur)
SELECT DATE '2025-02-01' + (g * 7), NULL,
       CASE WHEN g % 2 = 0 THEN 'Filler Property A' ELSE 'Filler Property B' END,
       'Management', 'Management Fee', 'Tenant', 'company',
       CASE WHEN g = 34 THEN 758.05 ELSE 762 END
