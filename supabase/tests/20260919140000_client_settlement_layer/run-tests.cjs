/* Isolated Postgres catalog/security/FIFO tests for 20260919140000.
...
const PORT = process.env.JJ_PG_PORT || '15436'
const CONTAINER = 'jj-client-settlement-pg'
const MIGRATION = path.join(REPO, 'supabase/migrations/20260919140000_client_settlement_layer.sql')
const FORBIDDEN = [
  '2944e9ad',
  '09e99dfa',
  '27546e91',
...
function assertMigrationGuards(sql) {
  ...
  if (/CREATE\s+OR\s+REPLACE\s+VIEW\s+public\.(v_cashbox_audit|v_certified_ledger_transactions|v_rc3_|v_contact_settlement_summary)/i.test(ddl)) {
